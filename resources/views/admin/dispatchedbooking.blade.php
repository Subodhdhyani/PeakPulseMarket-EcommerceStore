<x-dashboard_common>
    <x-slot:dynamic_title_top>
        Disaptached/Shipped Booking - PeakPulseMarket
        </x-slot>
        <x-slot:section_dynamic_content>
            <div class="container">
                {{--Heading--}}
                <h1 class="text-center text-danger">Manage Disaptached/Shipped Booking</h1>
            ...
            <script>
                $(document).ready(function() {
                    function loaddata() {
                        ...
                        $.ajax({
                            url: "{{route('dispatched_fetch_display')}}",
                            type: 'GET',
                            success: function(response) {
                                var respo = response.data;
                                $('#fetch_booking').empty();
                                if (respo.length === 0) {
                                    $('#fetch_booking').append('<tr><td colspan="6" class ="text-center text-danger" style="font-size: 30px;">No Dispatched/Shipped Booking/Order found</td></tr>');
                                } else {
