<x-dashboard_common>
    <x-slot:dynamic_title_top>
        Return Request Details - PeakPulseMarket
        </x-slot>
        <x-slot:section_dynamic_content>
            ...
            <div style="width: 80%; margin: auto; text-align: center; height: 400px; max-height: 90vh;">
                <h2 class="text-danger mt-4 mb-4">User Order Return Request Chart</h2>
            ...
            <script>
                ...
                const totalreturnrequest = "{{ $totalreturnrequest }}";
                const totalreturngenerated = "{{ $totalreturngenerated}}";
                const totalreturnreceived = "{{ $totalreturnreceived }}";

                const returnChart = new Chart(ctx, {
                    type: 'bar', // change the type (e.g., 'line', 'pie', etc.)
                    data: {
                        labels: ['Return Requested', 'Return Process ', 'Return Received'],
                        datasets: [{
                            label: 'Total Order Return Request',
                            data: [totalreturnrequest, totalreturngenerated, totalreturnreceived],
                            backgroundColor: [
