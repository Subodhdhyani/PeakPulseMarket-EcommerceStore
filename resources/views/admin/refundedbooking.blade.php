<x-dashboard_common>
    <x-slot:dynamic_title_top>
        Refunded Booking - PeakPulseMarket
        </x-slot>
        <x-slot:section_dynamic_content>
            ...
            <div class="container">
                {{--Heading--}}
                <h1 class="text-center text-danger">Refunded Booking</h1>
            ...
            <script>
                $(document).ready(function() {
                    function loaddata() {
                        ...
                        $.ajax({
                            url: "{{route('refunded_fetch_display')}}",
                            type: 'GET',
                            success: function(response) {
                                var respo = response.data;
                                $('#fetch_booking').empty();
                                if (respo.length === 0) {
                                    $('#fetch_booking').append('<tr><td colspan="6" class ="text-center text-danger" style="font-size: 30px;">No Refunded Booking/Order Found</td></tr>');
                                } else {
                                    respo.forEach(function(record) {
                                        $('#fetch_booking').append(
                                            ...
                                            '<td class="d-none d-md-table-cell">' + record.billing_email + '</td>' +
                                            '<td class="d-none d-md-table-cell">' + record.order_status + '</td>' +
                                            '<td>' +
                                            '<a href="" class="refunded_booking_action text-dark" data-id="' + record.booking_id + '" data-bs-toggle="tooltip" title="Delete Refunded Booking"><i class="bi bi-archive-fill"></i></a>&emsp;' +
                                    ...
                                    $('.refunded_booking_action').click(function(e) {
                                        e.preventDefault();
                                        ...
                                        swal({
                                            ...
                                            if (willDelete) {
                                                //deletion via AJAX
                                                $.ajax({
                                                    url: '/admin/delete_refunded_booking/' + recordId,
                                                    type: 'DELETE',
                                                    headers: {
