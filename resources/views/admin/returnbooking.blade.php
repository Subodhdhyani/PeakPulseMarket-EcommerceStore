<x-dashboard_common>
    <x-slot:dynamic_title_top>
       Returning Booking - PeakPulseMarket
        </x-slot>
        <x-slot:section_dynamic_content>
            ...
            <div class="container">
                {{--Heading--}}
                <h1 class="text-center text-danger">Manage Returning Booking</h1>
                    ...
                    <table class="table  table-striped table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" class="d-none d-md-table-cell">Order Status</th>
                                <th scope="col"><span class="d-none d-sm-table-cell">Return </span>Pickup</th>
                                <th scope="col"><span class="d-none d-sm-table-cell">Return </span>Received</th>
                            </tr>
                        </thead>
                ...
                <div class="modal-dialog">
                    <div class="modal-content" style="background-color:antiquewhite">
                        <div class="modal-header  text-center">
                            <h1 class="modal-title fs-5 w-100" id="staticBackdropLabel">Add Tracking Details for Return Pickup</h1>
                        ...
                        <div class="modal-body">
                            <form autocomplete="off" id="myform">
                                ...
                                <hr>
                                <button type="button" id="add_track_details" class="btn btn-warning d-block mx-auto">Add Tracking Details and Return Pickup</button>
                            </form>
                        </div>
                    </div>
                </div>
            ...
            {{--Add jquery,sweet alert  and toastr--}}
            ...
            <script>
                $(document).ready(function() {
                    function loaddata() {
                        ...
                        $.ajax({
                            url: "{{route('return_fetch_display')}}",
                            type: 'GET',
                            success: function(response) {
                                var respo = response.data;
                                $('#fetch_booking').empty();
                                if (respo.length === 0) {
                                    $('#fetch_booking').append('<tr><td colspan="7" class ="text-center text-danger" style="font-size: 30px;">No Return Booking/Order found</td></tr>');
                                } else {
                                    respo.forEach(function(record) {
                                        $('#fetch_booking').append(
                                            ...
                                            '<td class="d-none d-sm-table-cell">' + record.billing_phone + '</td>' +
                                            '<td class="d-none d-md-table-cell">' + record.order_status + '</td>' +
                                            '<td>' +
                                            '<a href="javascript:void(0);" class="return_booking_action text-dark" data-bs-toggle="modal" data-bs-target="#targetmodal" data-id="' + record.booking_id + '" data-bs-toggle="tooltip" title="Add Return Tracking/Pickup"><i class="bi bi-truck"></i></a>' +
                                            '</td>' +
                                            '<td>' +
                                            '<a href="javascript:void(0);" class="return_received_action text-success" data-id="' + record.booking_id + '" data-bs-toggle="tooltip" title="Mark as Received">' +
                                    ...
                                    $(document).on('click', '.return_booking_action', function(e) {
                                        e.preventDefault();
                                    ...
                                    $('#add_track_details').click(function(e) {
                                        ...
                                        $.ajax({
                                            url: "{{route('return_detail_add_track')}}",
                                            type: 'POST',
                                            data: {
                                                bookingId: bookingId,
                                    ...
                                    //Mark as Received Return
                                    $(document).on('click', '.return_received_action', function(e) {
                                        e.preventDefault();
                                        $('.spinner-container').show(); //show spinner
                                        var bookingId = $(this).data('id'); // Get the booking_id
                                        // SweetAlert confirmation
                                        swal({
                                            title: "Are you sure?",
                                            text: "To mark the Product as Received !",
                                            icon: "warning",
                                            buttons: true,
                                            dangerMode: true,
                                        }).then((willDelete) => {
                                            if (willDelete) {
                                                //deletion via AJAX
                                                $.ajax({
                                                    url: "{{route('return_mark_received')}}",
                                                    type: 'POST',
                                                    data: {
                                                        bookingId: bookingId
                                                    },
                                                ...
                                                swal("Cancelled", "Marked the Product as Received !", "info");
                                                $('.spinner-container').hide(); //hide the spinner
                                            }
                                        });
                                    });
