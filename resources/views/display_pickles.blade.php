...
<head>
    ...
    <title>Pickles - PeakPulseMarket</title>
...
<body>
    ...
    <div class="dynamic-search-result-show-then-hide-this">{{--Inside navbar Component--}}
        ...
        <h2 class="text-center mt-4 mb-4" style="color: #1fdb22;">Explorer Pickles <i class="fa-solid fa-angles-down fa-lg" style="color: #1fdb22;"></i></h2>
            ...
            <div class="card-template" style="display: none;">
                <div class="card">
                    <a href="" target="_blank">
                        <img src="{{ asset('category/pickle.png') }}" class="card-img-top" alt="product_card_image">
                        <hr>
                        <div class="card-body">
    ...
    <script>
        $(document).ready(function() {
                    ...
                    if (response.status === 'success') {
                        ...
                        if (fetch.length === 0) {
                            ...
                            fetch.forEach(function(record) {
                                ...
                                //$cardTemplate.find('.card-text').text(record.description.substring(0, 35));
                                ...
                                //fetchProductRating(record.id, $cardTemplate);

                                // Fetch rating for the product using POST request
                                //fetchProductRating(record.id, $cardTemplate);
            ...
            //Now fetch rating combined when fetch product details
            /* Function to fetch the rating for each product
            function fetchProductRating(productId, $cardTemplate) {
                $.ajax({
                    url: '{{-- route("fetch_rating", ":id") --}}'.replace(':id', productId), // Replace :id with actual product ID
                    type: 'GET',
                    success: function(ratingResponse) {
                        if (ratingResponse.status === 'success') {
                            var rating = ratingResponse.message;
                            $cardTemplate.find('.rating-fetch').text(rating);
                        } else if (ratingResponse === 'error') {
                            $cardTemplate.find('.rating-fetch').text('No Rating');
                        }
                    },
                    error: function() {
            ...
            // Function to fetch the rating for each product using POST request
            /*function fetchProductRating(productId, $cardTemplate) {
                $.ajax({
                    url: '{{-- route("fetch_rating") --}}',
                    type: 'POST',
                    data: {
                    ...
                    success: function(ratingResponse) {
                        if (ratingResponse.status === 'success') {
                            $cardTemplate.find('.rating-fetch').text(ratingResponse.message);
                        } else {
                            $cardTemplate.find('.rating-fetch').text('No Rating');
                        }
                    },
